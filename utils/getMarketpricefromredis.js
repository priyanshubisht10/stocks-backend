...
const getLatestMarketPrice = async (stockSymbol) => {
  ...
  if (latestEntry.length) {
    ...
    console.log(
      `got amount from redis ${stockSymbol} : ${latestPriceData.price}`
    );
  ...
  console.warn(
    `⚠️ No market price found in Redis for ${stockSymbol}, checking DB...`
  );
  ...
  if (!stock || !stock.current_price) {
    return new AppError(
      `Market price unavailable for ${stockSymbol}. The stock might not be listed yet.`,
      500
    );
  }
